...
DFT_Module_reel_imag proc
	...
	mov R6,#0 ;R6 correspond à l'index n
	mov R4,#0 ;R4 correspond au résultat
	
Deb_boucle	;debut boucle
	cmp R6,#64
	beq End_reel
...
DFT_module_carre proc
	push {R4-R11, lr}
	mov R4,R0 ; R4 = dma_buf
	mov R5,R1 ; R5 = k
	ldr R2,=TabCos
	bl DFT_Module_reel_imag
	mov R6,R0 ;R6 = partie réelle
	mov R0,R4
	mov R1,R5
	ldr R2,=TabSin
	bl DFT_Module_reel_imag
	...
	mul R0,R0
	mul R1,R1
	;R1 et R0 format 10,20
	asr R0,#1
	asr R1,#1
	;R0 et R1 format 10,19 et donc 13,19 en vrai car registres 32 bits
	add R0,R1
	pop {R4-R11, lr}
	bx lr
	endp
